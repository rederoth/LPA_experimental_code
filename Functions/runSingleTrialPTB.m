function td = runSingleTrialPTB(el, td)
%RUNSINGLETRIAL Summary of this function goes here
% This version of runSingleTrial does not use matlabs VideoReader and will
% hence avoid codec problems (i.e.  usable with other ubuntu versions etc).

% Adapted by Nicolas Roth
...
if td.fixReq == 0 && td.image_file_exists == 1 && td.do_escape == 0
    ...
    if td.video == 1 && td.do_escape == 0
        ...
        if exist(td.moviename, 'file') % check whether the file exists
            ...
            disp([num2str(GetSecs), td.moviename]);
            [movie, movieduration, fps, imgw, imgh, count, ~, ~] = Screen('OpenMovie', scr.main, td.moviename);
            numFrames = count;
            disp([num2str(GetSecs), ' Count returns ', num2str(count)]);
            td.moviedata = [movieduration, fps, imgw, imgh, numFrames, NaN, td.v_width, td.v_height];
            t_preload_end = GetSecs;
            disp([num2str(GetSecs), ' Loading took ', num2str(round(t_preload_end-t_reading_start, 3)), ' secs.']);
        ...
        for frame_i = 1:td.n_flips
            tex = Screen('GetMovieImage', scr.main, movie);
            if tex < 0
            ...
            Screen('DrawTexture', scr.main, tex, [], td.video_dest_rect);
...
%             if frame_i == 1
%                 td.t_flip_first_video = Screen('Flip', scr.main, td.t_flip_image + identicalSecs);
%                 if setting.TEST == 0
%                     Eyelink('message', 'EVENT_imageEnd_videoStart');
%                 end
%                 disp([num2str(GetSecs), ' Image presentation time: ', num2str(round(td.t_flip_first_video- td.t_flip_image, 3)) ' secs.']);
